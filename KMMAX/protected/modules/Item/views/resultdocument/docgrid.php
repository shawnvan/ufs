...
$('#batchdownload').attr('href',$('#batchdownload').attr('href')+'/id/".$itemno."');
$('#CreateDoc').attr('href',$('#CreateDoc').attr('href')+'/id/".$itemno."');
          function apply(rid){
				  jQuery.ajax({				
				  url:'".Yii::app()->createUrl('Item/resultdocument/apply')."/cno/'+treeId+'/id/'+jQuery('#hiddenpkey').val(),
				  cache: false,
	...
	function zTreeClickNode(treeId,treeParentId, treeName,noteid) {
		...
		jQuery.ajax({				
		  url:'".Yii::app()->createUrl('Item/resultdocument/docGridData')."/cno/'+treeId+'/id/'+jQuery('#hiddenpkey').val(),
		  cache: false,
		  success: function(html){
		  	 var urlindex='".Yii::app()->createUrl('Item/resultdocument/docGridData')."';
			 var url=urlindex +'/cno/'+treeId +'/id/'+jQuery('#hiddenpkey').val();
	...
	$('.batchdownload').live('click',function(){ 
	   var url = '".Yii::app()->createUrl('Item/resultdocument/batchdownload')."/id/".$itemno."';
	    $(this).attr('href',url);
		$(this).colorbox({iframe:true, width:'100%', height:'100%',onClosed: function (message) {}});
  		return false;
     });
...
<?php $this->renderPartial('_searchDoc',array(
	'model'=>$model,
	     ...
	     $this->widget('application.modules.UFSBase.utils.WGrid',array(
			        ...
			        'columnsModel'=>array(
						array('name'=>'fAttachmentNo','frozen'=>true),
