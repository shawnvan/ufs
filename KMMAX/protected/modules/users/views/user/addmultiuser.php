		...
		if(selectedRowId=='') {alert('您未选择任务标准任务');return false;}
	     ...
	     var insertUserMail='';
              	     for(var i=0;i<selectedRowId.length;i++) {
                  	    ...
                  	    if(insertUserNo=='')
                  	    {
		                  insertUserNo=ret.fUserName;
		                  insertUserMail=ret.fEmail;
                         }
		                else 
		                {
		                  insertUserNo=insertUserNo+','+ret.fUserName;
						  insertUserMail=insertUserMail+','+ret.fEmail;
                        }
         }
		parent.$('#Msgfrom_fSendToAccount').val(insertUserMail);
		parent.$('#Msgfrom_fSendToName').val(insertUserNo);
