...
/**
 * This is the model class for table "tbl_userlogin".
 *
 * The followings are the available columns in table 'tbl_userlogin':
 * @property integer $fLogID
 * @property string $fUserID
 * @property string $fLoginDate
...
class Userlogin extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fCreateUser, fCreateDate', 'required'),
			array('fUserID, fCreateUser, fUpdateUser', 'length', 'max'=>25),
			array('fLoginIP', 'length', 'max'=>20),
			array('fLoginDate, fUpdateDate', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('fLogID, fUserID, fLoginDate, fLoginIP, fCreateUser, fCreateDate, fUpdateUser, fUpdateDate', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('fLogID',$this->fLogID);
		$criteria->compare('fUserID',$this->fUserID,true);
		$criteria->compare('fLoginDate',$this->fLoginDate,true);
