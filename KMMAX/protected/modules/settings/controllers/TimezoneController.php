...
class TimezoneController extends AppController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Timezone']))
		{
			...
			if($model->save())
				$this->redirect(array('view','id'=>$model->fTimeZoneID));
		}
    ...
    public function actionIndex()
    {
        ...
        $pages=new CPagination(Timezone::model()->count($criteria));//记录总数
        $pages->pageSize=5;//设置每页的记录显示条数
        $pages->applyLimit($criteria);
		
        $sort=new CSort('Timezone');//排序，参考YII文档CSort
        $sort->attributes=array(
        			'fTimeZoneID'=>array('asc'=>'fTimeZoneID','desc'=>'fTimeZoneID desc','label'=>Timezone::model()->getAttributeLabel('fTimeZoneID')),
		'fTimeZoneName'=>array('asc'=>'fTimeZoneName','desc'=>'fTimeZoneName desc','label'=>Timezone::model()->getAttributeLabel('fTimeZoneName')),
        );
        		 $sort->defaultOrder='fTimeZoneID';
        $sort->applyOrder($criteria);

        // find all
        $models=Timezone::model()->findAll($criteria);
        ...
        {
            $gridRows[]=array(
            			 array('content'=>CHtml::encode($model->fTimeZoneID)),
		 array('content'=>CHtml::encode($model->fTimeZoneName)),
            );
        }	
    ...
    {  	
        ...
        {
            $field=array(
						'fTimeZoneID'=>Timezone::model()->getAttributeLabel('fTimeZoneID'),
		'fTimeZoneName'=>Timezone::model()->getAttributeLabel('fTimeZoneName'),
            );
		...
		$pages=new CPagination(Timezone::model()->count($criteria));
        $pages->pageSize=$jqGrid['pageSize']!==null ? $jqGrid['pageSize'] : 5;
        $pages->applyLimit($criteria);
        // sort
        $sort=new CSort('Timezone');
        ...
        foreach($models as $model)
        {
            ...
            $data['rows'][]=array(
                		 'fTimeZoneID'=>$model->fTimeZoneID,'cell'=>array(CHtml::encode($model->fTimeZoneID).CHtml::link("<span class='ui-icon ui-icon-zoomin'></span>",array('view','id'=>$model->fTimeZoneID),array(
                    'class'=>'UFSGrid-show UFSGrid-row-button',
                    'align'=>'right',
					'title'=>'show'
                )).CHtml::link("<span class='ui-icon ui-icon-pencil'></span>",array('update','id'=>$model->fTimeZoneID),array(
                    'class'=>'UFSGrid-edit UFSGrid-row-button',
					'align'=>'right',
                    'title'=>'edit'
                )),		 CHtml::encode($model->fTimeZoneName),
            ));
        }
        $this->printJson($data);
    }
