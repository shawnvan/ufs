...
class AreaController extends AppController
{
	...
	{
		$model = Area::model()->getCityList($pid);
        foreach($model as $value=>$name)
        {
            echo CHtml::tag('option',array('value'=>$value,'data-area'=>$name),CHtml::encode($name),true);
        }
	}
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Area']))
		{
			...
			if($model->save())
				$this->redirect(array('view','id'=>$model->fAreaID));
		}
    ...
    public function actionIndex()
    {
        ...
        $pages=new CPagination(Area::model()->count($criteria));//记录总数
        $pages->pageSize=5;//设置每页的记录显示条数
        $pages->applyLimit($criteria);
		
        $sort=new CSort('Area');//排序，参考YII文档CSort
        $sort->attributes=array(
        			'fAreaID'=>array('asc'=>'fAreaID','desc'=>'fAreaID desc','label'=>Area::model()->getAttributeLabel('fAreaID')),
		'fAreaName'=>array('asc'=>'fAreaName','desc'=>'fAreaName desc','label'=>Area::model()->getAttributeLabel('fAreaName')),
		'fParentID'=>array('asc'=>'fParentID','desc'=>'fParentID desc','label'=>Area::model()->getAttributeLabel('fParentID')),
        );
        		 $sort->defaultOrder='fAreaID';
        ...
        {
            $gridRows[]=array(
            			 array('content'=>CHtml::encode($model->fAreaID)),
		 array('content'=>CHtml::encode($model->fAreaName)),
		 array('content'=>CHtml::encode($model->fParentID)),
            );
        }	
    ...
    {  	
        ...
        {
            $field=array(
						'fAreaID'=>Area::model()->getAttributeLabel('fAreaID'),
		'fAreaName'=>Area::model()->getAttributeLabel('fAreaName'),
		'fParentID'=>Area::model()->getAttributeLabel('fParentID'),
            );
		...
		$pages=new CPagination(Area::model()->count($criteria));
        ...
        foreach($models as $model)
        {
            ...
            $data['rows'][]=array(
                		 'fAreaID'=>$model->fAreaID,'cell'=>array(CHtml::encode($model->fAreaID).CHtml::link("<span class='ui-icon ui-icon-zoomin'></span>",array('view','id'=>$model->fAreaID),array(
                    'class'=>'UFSGrid-show UFSGrid-row-button',
                    'align'=>'right',
					'title'=>'show'
                )).CHtml::link("<span class='ui-icon ui-icon-pencil'></span>",array('update','id'=>$model->fAreaID),array(
                    'class'=>'UFSGrid-edit UFSGrid-row-button',
					'align'=>'right',
                    'title'=>'edit'
                )),		 CHtml::encode($model->fAreaName),
		 CHtml::encode($model->fParentID),
            ));
        }
