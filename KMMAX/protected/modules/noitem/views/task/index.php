...
$('.UFSGrid-row-delete').live('click',function(){ 
        if(confirm('您确定要删除吗？')){
    		jQuery.ajax({				
				  url:'".Yii::app()->createUrl('noitem/task/delete/id')."/'+$(this).attr('rel'),
				  type:'POST',
				  success: function(data){
				    var urlindex='".Yii::app()->createUrl('noitem/task/gridData')."';
...
<input type="hidden" id=treeId name="hiddenpkey" />
	...
	<?php        $this->widget ( 'zii.widgets.CMenu', array ('id'=>'',
                ...
                'items' =>array(
                		array('label'=>Yii::t('label','List'), 'url'=>array('index'),'linkOptions'=>array('id'=>'ListTask','class'=>'current'),'visible'=>Yii::app()->user->checkAccess('noitem.task.Index')),
...
<?php     $this->widget('application.modules.UFSBase.utils.WGrid',array(
       ...
       'sortname'=>'fTaskNo',
        'sortorder'=>'asc',
        'url'=>Yii::app()->createUrl('noitem/task/gridData',$_GET),
        'modulename'=>'Task',
    )); ?>
</div>