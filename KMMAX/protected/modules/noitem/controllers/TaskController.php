...
class TaskController extends FatherTask
{
	...
	{
		$model=Task::model()->with('knowcatalogue')->findByPk($id);
		if(!empty($model)){				
    ...
    public function actionIndex()
    {
        ...
        $this->render('index',array(
            ...
            'sort'=>$sort,
            'gridRows'=>$gridRows,
            'model'=>$model,'keyid'=>'','dataNode'=>$dataNode,
        ));
    }
    ...
    {  	
       ...
       if($jqGrid['filters']!=null){
        ...
        $sort->defaultOrder='fTaskNo';
        $sort->applyOrder($criteria);
        $models=Task::model()->with('knowcatalogue')->findAll($criteria);
        $data=array(
        ...
        {        	
            $data['rows'][]=array(
						...
						'cell'=>array(CHtml::encode($model->fTheme).(Yii::app()->user->checkAccess('noitem.task.Update')?CHtml::link("<span class='ui-row ui-row-edit'></span>",array('update','id'=>$model->fTaskNo),array(
                    'class'=>'UFSGrid-edit UFSGrid-row-button',
					'align'=>'right',
                    'title'=>Yii::t('label','Update')
                )):'').(Yii::app()->user->checkAccess('noitem.task.View')?CHtml::link("<span class='ui-row ui-row-view'></span>",array('view','id'=>$model->fTaskNo),array(
                    'class'=>'UFSGrid-show UFSGrid-row-button',
                    'align'=>'right',
					'title'=>Yii::t('label','View')
                )):'').(Yii::app()->user->checkAccess('noitem.task.Delete')?CHtml::link("<span class='ui-row ui-row-delete'></span>",array('delete','id'=>$model->fTaskNo),array(
		 ...
		 CHtml::encode(empty($model->knowcatalogue->fCatalogueName)?'':$model->knowcatalogue->fCatalogueName),
	...
	public function actionTaskhistory(){
		...
		if(isset($_GET['id'])){
			$model=Task::model()->with('knowcatalogue')->findByPk($_GET['id']);
	...
	public function GetTaskHandle($model){
		$taskstop=CHtml::submitButton('终 止',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskstop/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskgoon=CHtml::submitButton('重新开放',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskgoon/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskagree=CHtml::submitButton('终 止',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskagree/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskfinish=CHtml::submitButton('完成',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskfinish/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskfinishandstandard=CHtml::submitButton('完成并推荐到标准库',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskfinish/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskfinishandstandard=CHtml::submitButton('完成并推荐到标准库',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskfinish/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskstandard=CHtml::submitButton('推荐到标准库',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/standard/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskrefusefinish=CHtml::submitButton('拒绝完成申请',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskrefusefinish/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskfinishnew=CHtml::submitButton('完成并新建',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskfinishnew/id/'.$model->fTaskNo,'align'=>'right','title'=>'edit','class'=>'submit'));
		$res='';
	...
	public function GetDownTaskHandle($taskno,$status){
		...
		$confirm=CHtml::submitButton('确认',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskconfirm/id/'.$taskno,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskfeedback=CHtml::submitButton('任务交互',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskfeedback/id/'.$taskno,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskagree=CHtml::submitButton('同意',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskagree/id/'.$taskno,'align'=>'right','title'=>'edit','class'=>'submit'));
		$taskrefuse=CHtml::submitButton('拒绝',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/taskrefuse/id/'.$taskno,'align'=>'right','title'=>'edit','class'=>'submit'));
		$middlefinished=CHtml::submitButton('任务完成提交',
				array('rel'=>'/UFS/KMMAX/index.php/noitem/task/middlefinished/id/'.$taskno,'align'=>'right','title'=>'edit','class'=>'submit'));
	...
	*/
	...
	$upload->allowExts  = array('jpg','gif','doc','txt','png','doc','txt','xls','docx','pdf','xlsx');
			$upload->savePath=Yii::app()->params['upload_path'];
