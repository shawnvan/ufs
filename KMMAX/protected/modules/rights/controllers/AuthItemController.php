...
class AuthItemController extends RController
{
	...
	*/
	...
	public function actionIndex()
	{
		...
		//获得模块
        $models=Authitem::model()->findAllByAttributes(array('type'=>'1'));
        $modelsResult=array();
        foreach ($models as $model){
        	array_push($modelsResult,$model->name);
        }

		//摸得模块操作
		$this->render('index',array(
				'orgchat'=>$orgchat,'modelsResult'=>$modelsResult
		));
	}
	/**
	...
	*/
	...
	public function ActionOperate($mou){
		$criteria=new CDbCriteria;
		$criteria->addCondition("type=:type");
		$criteria->params[':type']=0;
		$criteria->select='name';
		$criteria->addCondition("name like '".strtr($mou,"*","%")."'"); //查询条件，即where id =1
		$moduleActions=Authitem::model()->findAll($criteria);
		$modelsResult=array();
		foreach ($moduleActions as $model){
			array_push($modelsResult,$model->name);
		}
		$this->renderPartial('update',array(
				'data'=>UFSBaseUtil::printJson($modelsResult))
		);
		return $mou;
	}
}
